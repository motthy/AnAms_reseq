...
apptainer exec --no-mount tmp -B $WORKDIR $GATK gatk --java-options "-Xmx4G" HaplotypeCaller \
...
apptainer exec --no-mount tmp -B $WORKDIR $GATK gatk --java-options "-Xmx4G" IndexFeatureFile \
     ...
     --output ${SRR_ACC}.g.vcf.gz.tbi

# GATK GenotypeGVCFs
apptainer exec --no-mount tmp -B $WORKDIR $GATK gatk --java-options "-Xmx4G" GenotypeGVCFs \
